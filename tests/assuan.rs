...
/// (a session that never prompts) are skipped.
...
fn cancel_timeout_and_repeat() {
    ...
    assert!(
        requests.recv().unwrap().get("error").is_none(),
        "SETERROR is one-shot"
    );
    assert_eq!(requests.recv().unwrap()["repeat"], "Again");
    assert!(
        requests.recv().unwrap().get("repeat").is_none(),
...
fn getinfo_and_unknown_commands() {
    ...
    assert_eq!(
        lines,
        ["D omarchy", "OK", "OK", "ERR 83886355 Unknown IPC command"]
    );
}
