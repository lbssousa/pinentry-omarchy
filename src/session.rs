//! The Assuan conversation with gpg-agent: reads commands, asks the dialog,
//! and answers.
...
use crate::request::{Kind, State};
use crate::shell::Dialog;

/// Serves one pinentry session: greets, then answers commands from `input`
/// on `output` until BYE or end of input.
pub fn serve(mut dialog: Dialog, mut input: impl BufRead, output: impl Write) -> io::Result<()> {
